...
/// <summary>
/// The versioned schema migrations PgWorkflows applies on startup (when
/// <c>ensureSchemaOnStart</c> is true). Applied versions are recorded in
/// <c>pw_schema_migrations</c>; only pending ones run.
/// </summary>
/// <remarks>
/// Exposed so deployments that run migrations themselves can apply them out-of-band
/// instead: execute each pending migration's <see cref="PostgresMigration.Sql"/> and
/// insert its row into <c>pw_schema_migrations(version, name, applied_at)</c> in the
/// same transaction.
/// </remarks>
public static class PostgresSchema
{
    /// <summary>Creates the bookkeeping table itself; safe to re-run.</summary>
    public static readonly string MigrationsTableSql = """
        create table if not exists pw_schema_migrations (
    ...
    /// <summary>All migrations, ordered by version.</summary>
    public static IReadOnlyList<PostgresMigration> Migrations { get; } =
    [
        // The baseline stays idempotent ("if not exists") so databases created before schema
        // versioning existed adopt it without error. Migrations after the baseline are plain DDL.
        new(
            1,
            "baseline",
            """
